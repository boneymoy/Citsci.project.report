...
%*****************************************
    ...
    In~\autoref{sec:Datenerfassung} wird dann auf den Ort der Erfassung und in~\autoref{sec:syscalls} auf die eigentliche Daten für die Überwachung, die System Calls, eingegangen.
    ...
    \section{Intrusion Detection System}\label{sec:IDS}
        Eine \textit{Intrusion} \marginpar{zu dt. Eindringen, Einbruch} ist eine unautorisierte Aktivität, welche einem System Schaden zufügen kann.
        ...
        {\color{red}alternativquelle (Buch?)}
        ...
        Das Erkennen von Angriffen auf Computersystemen mittels \ac{IDS} wird im wesentlichen in zwei Kategorien eingeteilt.~\cite{IDSreview, IDSsurvey, IDSsurvey2}.
            ...
            \paragraph{Signaturbasiert} 
                ...
                Dafür werden zum Beispiel in mehreren Arbeiten Netzwerk Pakete betrachtet~\cite{IDSsurvey}
                ...
                Weswegen im Folgenden die anomaliebasierte Ermittlung von Intrusions beschrieben wird.
            ...
            \paragraph{Anomaliebasiert}
                ...
                Das Normalverhalten oder anders formuliert das Modell eines Systems im Normalzustand soll möglichst akkurat das zu erwartende Systemverhalten beschreiben.~\cite{ANOMALYBOOKKISHAN2017}
                Anomalien dieses Verhaltens können dann zu signifikanten und oft kritischen Veränderungen eines Systems führen.
                ...
                Anomalien oder auch \textit{\glqq outlier\grqq\ }\marginpar{zu dt. Ausreißer} wurden unter anderem schon 1969 von Grubbs definiert.
                Wobei in der Arbeit von Grubbs versucht wird diese Ausreißer aus einem Trainingsdatensatz zu entfernen.

                \glqq An outlying observation, or outlier, is one that appears to deviate markedly from other members of the sample in which it occurs\grqq\ ~\cite{ANOMALYDEFINITION1969}

                Anomalien in Daten von Computersystemen können aus verschiedenen Gründen entstehen, zum Beispiel durch böswillige Aktivitäten oder aber auch durch Programmfehler.
                ...
                Dabei können \ac{ML}-basierte, Statistik-basierte oder auch 
        ...
        \subsection{Datenerfassung}\label{sec:Datenerfassung}
            ...
            \paragraph{Host-Daten zur Beschreibung des Systemverhaltens}
                ...
                Nach Bridges et al.\ besteht im wesentlichen die Auswahl zwischen Log-Files und \textit{System Calls}\marginpar{zu dt. Systemaufrufe}. 
                ...
                Beide Log Arten beschreiben das Systemverhalten es findet aber bereits eine starke Filterung statt.
                Außerdem müssen die angesprochenen Filter manuell eingestellt werden.
                ...
                in die anomaliebasierten und signaturbasierten Verfahren eingeteilt wird.{\color{red}Grafik anpassen}
    ...
    \section{System Calls}\label{sec:syscalls}
        ...
        Wie in \autoref{tab:syscall} ebenfalls beschrieben besitzen System Calls auch Argumente die beim Aufrufen mitgegeben werden können.        
        ...
        Schließende System Calls haben zusätzlich in den Argumenten einen Rückgabewert~\marginpar{Gekennzeichnet mit \glqq $res$\grqq}.
        ...
        Auch ein möglicher Angreifer muss um Schaden anzurichten also an den System Calls eine Veränderung vornehmen.
        ...
        \subsection{Datensätze}\label{sec:Datensatz}
            ...
            \subsubsection{LID-DS}
                ...
                In~\autoref{tab:runsfile} soll zunächst die beschreibende $runs.csv$ betrachtet werden und anschließend in~\autoref{tab:syscallfile} eine Beispieldatei einer Aufzeichnung.
                \begin{table}[ht]
                    \tiny
                    \centering
                ...
                \paragraph{Problem des Datensatzes}
                    ...
                    Also könnte ein Alarm nach $t_{angriff}$ theoretisch auch ein \textit{False Positive} sein. 
                    Genauso könnte umgekehrt ein Anomaliescore unter dem Schwellwert korrekt sein obwohl er als \textit{False Negative} gewertet werden müsste.
                    Diese Problematik muss in der Auswertung beachtet werden und wird in~\autoref{sec:metrik} beleuchtet.
