...
In den folgenden Abschnitten sollen nun Grundlagen betrachtet werden, welche bei der Umsetzung des \ac{IDS} nötig sind.
    ...
    \section{Intrusion Detection System}\label{sec:IDS}
        ...
        \end{itemize}
        Die letztere Frage soll speziell in Kapitel~\ref{sec:Algorithmus} beleuchtet werden, doch wie das Systemverhalten präzise dargestellt werden kann soll nun behandelt werden.
    ...
    \section{System Calls}\label{sec:syscalls}
        ...
        Mögliche Services sind unter anderem Zugriffe auf Hardware aber auch das Erstellen und Ausführen von Prozessen oder das Setzen von File-Berechtigungen.
        ...
        So stehen den Nutzerprozessen eine limitierte Auswahl an privilegierten Funktionen aus dem Kernel zur Verfügung.
        Diese werden in der folgenden Aufzählung zusammengefasst.~\cite{SYSCALL_SILBERSCHATZ}
        \begin{itemize}
            \item Prozesskontrolle
            \begin{itemize}
                \item Erstellen, beenden, ausführen, \dots
            \end{itemize}
            \item File Management
            \begin{itemize}
                \item Erstellen, öffnen, schließen, löschen, \dots
            \end{itemize}
            \item Geräte Management
            \begin{itemize}
                \item Anfragen, lesen, \dots
            \end{itemize}
            \item Informationen pflegen
            \begin{itemize}
                \item Systeminformationen anfordern, Zeit und Datum erfragen, \dots
            \end{itemize}
            \item Kommunikation
            \begin{itemize}
                \item Erstellen und löschen von Kommunikationsverbindungen
                \item Senden und erhalten von Nachrichten 
                \item \dots
            \end{itemize}
        \end{itemize}

        Wie in \autoref{tab:syscall} ebenfalls beschrieben besitzen System Calls auch Argumente die beim Aufrufen mitgegeben werden können.        Dabei kommen neben bestimmten Flags welche die Funktion der System Calls bestimmen auch zum Beispiel Dateipfade oder auch Verweise auf IP-Adressen. 
        {\color{red}Zu jedem öffnenden System Call gibt es einen schließenden System Call, der Quasi die Antwort des Kernels darstellt.}
        ...
        \subsection{System Calls für IDS}
            ...
            Viele verschiedene \ac{IDS} Ansätze betrachten die Sequenz von System Calls und erzielen vielversprechende Ergebnisse.~\cite{MAGGI}{\color{red} richtige Quelle einfügen}%TODO Quelle einfügen
