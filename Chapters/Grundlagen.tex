...
%*****************************************
    ...
    Zu Beginn wird in~\autoref{sec:Datenanalyse} besprochen, welche Analysetechniken für die Überwachung von Systemen zur Verfügung stehen.
    ...
    wird im Anschluss die Grundlagen des in der Arbeit verwendeten Algorithmus untersucht.
    Dazu werden in~\autoref{sec:KNN} \acp{RNN}, sowie die Erweiterung der \acp{RNN} die \acp{LSTM} beschrieben.

    \section{\NoCaseChange{\acfp{IDS}}}\label{sec:IDS}
    \sectionmark{IDS}
        ...
        Ein \ac{IDS} überwacht Ressourcen, wie zum Beispiel Netzwerkdaten und versucht automatisch Missbrauch und anomales Verhalten zu identifizieren, um dadurch die Sicherheit des Systems gewährleisten zu können.~\cite{IDSPIETRO2008}\par\medskip

        Um solche unautorisierten Aktivitäten erkennen zu können, müssen Daten erfasst und anschließend analysiert werden.
        ...
        \subsection{Datenanalyse}\label{sec:Datenanalyse}
            ...
            \paragraph{Signaturbasiert} 
                ...
                und in wenigen auch \textit{State Machines} erstellt~\cite{SIDSstate}\marginpar{zu dt. Zustandsautomaten}.\par\medskip

                \ac{SIDS} haben meist eine sehr hohe Genauigkeit, doch ein wesentlicher Nachteil der signaturbasierten IDS liegt darin, dass keine neuartigen Angriffe erkannt werden.
                Auch Abwandlungen von schon bekannten Angriffen werden nicht erkannt.
                ...
                Anomaliebasierte \acp{IDS} adressieren diese Probleme.
            ...
            \paragraph{Anomaliebasiert}
                ...
                Das Normalverhalten oder anders formuliert das Modell eines Systems im Normalzustand beschreibt möglichst akkurat das zu erwartende und damit gewollte Systemverhalten~\cite{ANOMALYBOOKKISHAN2017}.
                Anomalien oder auch \textit{\glqq outlier\grqq\ }\marginpar{zu dt. Ausreißer} wurden unter anderem schon 1969 definiert.\par\medskip

                \blockquote{An outlying observation, or outlier, is one that appears to deviate markedly from other members of the sample in which it occurs.}~\cite{ANOMALYDEFINITION1969}\par\medskip
                ...
                So kann eine Anomalie in Netzwerkdaten dafür stehen, dass ein korrumpierter Computer sensitive Daten an ein unautorisiertes Ziel sendet~\cite{ANOMALYEXAMPLE}.
                %Festzuhalten ist also, dass sie das Anomalien eines  Normalverhaltens darstellen und damit interessant für Analysen sind.
                Genau dieser signifikante Unterschied des aktuellen Systemzustand zu einem entworfenen Modell (wohldefiniertes Normalverhalten) zu einem bestimmten Zeitpunkt wird dann als Anomalie eingestuft.
                Jede Anomalie gilt dann wiederum als \textit{Intrusion}.
                Zusammengefasst besteht also die Grundannahme dieser Methode darin, dass Intrusions von dem gelernten Normalverhalten des Systems unterschieden werden können.\par\medskip
                ...
                Dieser Schritt wird in \ac{ML}-basierte, Statistik-basierte~\cite{IDSPIETRO2008} oder~\cite{IDSsurvey} in \textit{Knowledge}-basierten \marginpar{zu dt. Wissens-basierte} Algorithmen untergliedert.
                In der zweiten Phase muss dieses Modell dann überprüft werden.
                Speziell wird mit noch nicht betrachteten Daten getestet, ob Normalverhalten und Anomalien auch als solche eingestuft werden.
                Dabei liefert der Algorithmus für jede Eingabe entweder einen Anomaliescore oder direkt ein Label.
                Das Label gibt an ob es sich um eine tatsächliche Intrusion handelt.
                Liegt der Score über einem festgelegten Schwellwert, gilt die Eingabe als anormal ansonsten als normal.~\cite{IDSPIETRO2008}\par\medskip

                Jedoch ergibt sich hier im Vergleich zu den \ac{SIDS} eine Problematik.
                Der erwähnte Schwellwert entscheidet über die Ergebnisqualität der anomaliebasierten \acp{IDS} und sollte daher sorgfältig und am besten automatisch ermittelt werden, um anwendungsspezifische Justierungen zu vermeiden.\marginpar{mehr dazu in~\autoref{sec:Algorithmus}}\par\medskip
                ...
                Diese unterscheiden sich hauptsächlich in der Anforderung an die Trainingsdaten. \par\medskip
                ...
                Auch deswegen ist diese Herangehensweise weniger verbreitet.~\cite{UNSUPERVISEDGOLDSTEIN2016}\par\medskip
                ...
                Die Grundidee besteht also darin, Abweichungen eines Modells des Normalverhaltens zu erkennen.\par\medskip
                ...
                Jedoch sind auch \textit{unsupervised} Trainingsverfahren umsetzbar, welche keine Anomalien in den Daten enthalten.\cite{UNSUPERVISEDGOLDSTEIN2016}\par\medskip
                Die Wahl des Algorithmus zur Erkennung von Anomalien in den Daten ist dabei entscheidend für den Ablauf der Trainingsphase und wird deshalb auch im Bezug auf das verwendete Verfahren in~\autoref{sec:Training} beschrieben.\par\medskip

                \newpage
                Insgesamt ergeben sich bei der Umsetzung der Anomaliedetektion allerdings einige Schwierigkeiten, welche im folgenden zusammengefasst werden:
        ...
        \subsection{Datenerfassung}\label{sec:Datenerfassung}
            ...
            \paragraph{Network Based Intrusion Detection}
                Die Grundidee besteht darin, dass ein Angriff immer den Zugriff auf die Computersysteme von außen erlangen möchte.
                Dafür wird der Netzwerkverkehr über Pakete, NetFlow oder andere Netzwerkdaten überwacht.
                Ein großer Vorteil ist, dass viele Computer in einem Netzwerk gleichzeitig überwacht werden.
                Ziel ist es , Angriffe möglichst früh zu erkennen und zu verhindern, dass sich die Gefahr weiter ausbreiten kann.
                Die Umsetzung von \ac{NIDS} ist bei besonders großen Netzen erschwert, da der hohe Datendurchsatz das Erkennen von Angriffen erschwert.~\cite{NIDS}\par\medskip
            ...
            \paragraph{Host Based Intrusion Detection}
                Wie der Name impliziert, konzentrieren sich \acp{HIDS} auf die Untersuchung von Daten, welche im Hostsystem entstehen. %KOMMA
                Es wird versucht das dynamische Verhalten, sowie den Zustand des Systems zu überwachen und dies nur mit Informationen, die auf dem Host zugänglich sind. %KOMMA
                In der Literatur werden hierfür verschiedene Informationsquellen genutzt.
                Dazu gehören Logs, z.B Firewall und Database Logs~\cite{IDSsurvey},
            ...
            \paragraph{Host-Daten zur Beschreibung des Systemverhaltens}
                ...
                In der Übersichtsarbeit wird Literatur besprochen, welche Windows Registry, File System, Programm Analysedaten untersuchen, welche im Folgenden aber nicht mit einbezogen werden.\par\medskip

                Log-Files können weiter aufgeteilt werden in System-Logs und programmspezifischen Logs, wobei System-Logs nur durch das Betriebssystem geschrieben werden.
                Beide Log Arten beschreiben das Systemverhalten, jedoch findet bei beiden bereits eine Filterung von Informationen des Normalverhaltens statt.%KOMMA
                Es muss manuell bestimmt werden, welche Aktionen überhaupt geloggt werden.
                Damit stellen sie deshalb meistens auch keine vertrauenswürdige Quelle dar~\cite{SEMANTICSCREECH2014}.
                ...
                Mit modernen Softwarelösungen wie sysdig~\cite{SYSDIG}, kann der Berechnungsaufwand für viele Anwendungsbereiche reduziert werden.
                In dieser Arbeit wird auf die Filterung durch Logs verzichtet und auf die detailliertere Darstellung des Systemverhaltens durch System Calls gesetzt.\par\medskip

                \autoref{fig:IDSOverview} gibt einen Überblick über die in den vorigen Abschnitten gemachte Einstufung.
                \begin{figure}[ht]
                    \centering
                ...
                in welcher die Datenerfassung in die Host-basierte und Netzwerk-basierte Erfassung unterteilt wird, sowie die Datenanalyse %KOMMA
                ...
                \end{itemize}
                Nachfolgend wird der Frage nachgegangen, wie das Systemverhalten durch Systemcalls präzise dargestellt werden kann. Die letzte Frage wird hingegen in~\autoref{sec:Algorithmus} beleuchtet.
    ...
    \section{System Calls}\label{sec:syscalls}
        ...
        Diese Interaktion findet in Form von \textit{System Calls}\marginpar{zu dt. Systemaufrufe} statt und kann mit einer gewöhnlichen Programm Schnittstelle verglichen werden.
        ...
        Die System Calls stellen dabei einen Kommunikationskanal zwischen dem Kernel und den darauf laufenden Programmen dar.
        ...
        In~\autoref{fig:syscallAblauf} wird der Ablauf dargestellt, in welchem ein Programm aus dem User space über einen System Call eine privilegierte Aktion ausführen.
        Dies ermöglicht es Nutzerprozessen auf eine limitierte Auswahl an privilegierten Funktionen aus dem Kernel zugreifen zu können.~\cite{SYSCALL_SILBERSCHATZ}\par\medskip
        ...
        Wie in~\autoref{fig:syscallAblauf} dargestellt, besteht ein System Call aus einem Aufruf an das Betriebssystem und der Antwort des Betriebssystems.
        In \autoref{tab:syscall} wird an zwei Beispielen beschrieben, welche System Calls Argumente beim Aufrufen mitgegeben werden können.        
        ...
        Zu einem System Call gehört also mehr als nur der Funktionsname.\par\medskip

        Ein Angreifer muss also, um Schaden anzurichten, Veränderungen an den System Calls vornehmen. %KOMMA
        Der folgende Abschnitt verschafft einen Überblick, wie die System Calls für ein \ac{IDS} verwendet werden können.
        %Denn eine Veränderung der System Call Sequenz kann auch verhindert werden.

        \subsection{System Calls für \NoCaseChange{\acp{IDS}}}
        \sectionmark{Sytem Calls für IDSs}
            Diverse \ac{IDS}-Ansätze betrachten die Abfolge der System Calls und erzielen vielversprechende Ergebnisse.~\cite{MAGGI}
            Häufig werden dabei aber nicht die in den Argumente der System Calls enthaltene Information mit einbezogen und bieten damit Möglichkeiten für unentdeckte Angriffe.
            Verschiedene Arbeiten~\cite{Syscallseqexploit1, Syscallseqexploit2, Syscallseqexploit3} zeigen, wie diese Angriffe durchgeführt werden können. 
            ...
            Was diese Beispiele jedoch zeigen ist, dass wenigstens einer dieser Faktoren, also entweder die Sequenz von System Calls, oder die Argumente der System Calls verändert werden muss.
            So kann zum Beispiel anstatt auf den Pfad \glqq /tmp/some/file\grqq \ auf \glqq /etc/passwd\grqq \ zugegriffen werden. 
            Welche dieser Argumente und wie diese genutzt werden können, um die Anomalieerkennung zu verbessern, wird in \autoref{sec:Meta} untersucht.
            In dem nachfolgenden Kapitel wird nun auf verschiedene Datensätze, welche System Call Sequenzen und teilweise Argumente und weitere Metadaten enthalten, eingegangen.
        ...
        \subsection{Datensätze}\label{sec:Datensatz}
            ...
            Dabei wird auf die Nutzbarkeit und die entstehenden Problematiken dieser für die \ac{HIDS} über System Calls eingegangen.
            \paragraph{DARPA}
                ...
                Eine der größten Kritikpunkte, welcher auch von Tavallaee et al.~\cite{KDDCRITIC2} und Engen~\cite{ENGEN2010} als solcher aufgefasst wird, besteht in der Redundanz der Aufnahmen.
            ...
            \paragraph{UNM}
                ...
                da er keine weiteren Kontextinformationen wie Thread IDs enthält~\cite{UNMcritic}.
            ...
            \paragraph{LID-DS}\label{sec:LIDDS}
                ...
                Dazu zählen unter anderem die Parameter, Rückgabewerte, Zeitstempel sowie User-, Prozess- und Thread-IDs.\par\medskip
                ...
                \begin{table}[]
                    ...
                    \begin{tabular}{p{3cm}p{6.5cm}}
                        ...
                        \hline
                    \end{tabular}
                    \caption[Kurzbeschreibung der Szenarien des \ac{LID-DS}]{Kurzbeschreibung der in dem Datensatz vorkommenden Szenarien und den dazugehörigen Sicherheitslücken~\cite{LIDDS}}
                    \label{tab:scenarien}
                \end{table}
                ...
                Jede dieser Aufzeichnungen ist in einer Datei gespeichert, welche in einer $runs.csv$ beschrieben und zusammengefasst werden.\par\medskip
                 
                In~\autoref{tab:runsfile} wird ein Ausschnitt aus der beschriebenen $runs.csv$ dargestellt und in~\autoref{tab:syscallfile} eine Beispieldatei einer tatsächliche Aufzeichnung von System Calls.
                \begin{table}[ht]
                    \small
                    \centering
                    \begin{tabular}{l | g  l}
                    ...
                    \end{tabular}
                    \caption[Ausschnitt der runs.csv des \ac{LID-DS}]{Ausschnitt aus der runs.csv des \ac{LID-DS}, welche unter anderem die Labels für Angriffsfiles und Normalfiles enthalten.~\cite{LIDDS}}
                    \label{tab:runsfile}
                \end{table}
                \begin{table}[ht]
                    \scriptsize
                    ...
                    \begin{tabular}{l | g  l  g}
                        ...
                        \hline
                    \end{tabular}
                    \caption[Ausschnitt aus dem \ac{LID-DS}]{Ausschnitt aus den eigentlichen Aufzeichnungen von System Calls aus dem \ac{LID-DS}~\cite{LIDDS}}
                    \label{tab:syscallfile}
                \end{table}

                \paragraph{Problem des LID-DS}\label{sec:prob_LIDDS}
                    ...
                    Oder falls fälschlicherweise ein Angriff erkannt wird, die Zuordnung als \acf{FP}.
                    Die Zuordnungen der Dateien mit Angriff werden in~\autoref{fig:quadrant} dargestellt.\par\medskip
            
                    \begin{figure}[ht]
                        \centering
                        \includegraphics[width=0.8\textwidth]{images/q_problem.eps}
                        \caption[Quadrantenproblem]{Illustration des Quadrantenproblems.
                        Es beschreibt die Problematik, dass Alle System Calls nach $t_{Angriff}$ als bösartig eingestuft werden müssen, obwohl auch Normalverhalten stattfindet. (Abwandlung von~\cite{IDSTHREADGRIMMER2021})}
                        \label{fig:quadrant}
                    \end{figure}
                    ...
                    Es wurde ein Alarm gemeldet, an einer Stelle an der (noch) kein Angriff stattfand.
                ...
                Nachdem verschiedene allgemeine Ansätze zur Erkennung von schädlichem Verhalten eines Systems und mögliche Datensätze zur Auswertung untersucht wurden,
                wird im Folgenden die Frage geklärt, wie mit Hilfe von künstlichen neuronalen Netzen Muster in einem Datensatz erkannt werden können.
    ...
    \section{Künstliche neuronale Netze}\label{sec:KNN}        
        ...
        Zu aktuellen Beispielen zählen dabei auch Vorhersagen die im Zusammenhang mit der Verbreitung des COVID-19 Virus stehen~\cite{COVID1, COVID2, COVID3}.\par\medskip
        ...
        Dabei haben sich spezialisierte Herangehensweisen entwickelt, wie zum Beispiel die \acp{CNN} für die Bilderkennung. 
        Für sequentielle Daten wie Audio, Text und Video, bei welchen eine zeitliche Komponente entscheidend ist, eignen sich vor allem die \acp{RNN}.
        Da in dieser Arbeit sequentielle Daten betrachtet werden, die sequentielle Abfolge von System Calls, wird im Folgenden nur auf die \acp{RNN} und eine besondere Abwandlungen dieser eingegangen.
        % überarbeiten
        ...
        \subsection{\acfp{RNN}}\label{sec:RNN}
        ...
        Dadurch wird es ermöglicht auch zeitlich abhängige Sequenzen zu erlernen, da die Signalverarbeitung der \acp{RNN} auch vorherige Geschehnisse mit einbezieht.
            Im Gegensatz dazu steht zum Beispiel die Bilderkennung, bei welchem das vorherige Bild keinen Einfluss auf die Einschätzung des aktuellen Bildes hat.\par\medskip

            Die folgende Gleichung stellt den Sachverhalt dar.
            \begin{equation}
                \begin{split}
                ...
                \end{split}
            \end{equation}
            $x_t$, $h_t$ und $y_t$ beschreiben den Eingang des Neurons, die rekurrente Information und den Ausgang des \acp{RNN}\@.
            ...
            Gewichte sowie der Bias werden für die Übersichtlichkeit nicht dargestellt.
                \begin{figure}[ht]
                    \centering
            ...
            Es kommt häufig vor, dass durch die \textit{Backpropagation} die berechneten Gradienten entweder verschwindend klein, oder sehr groß werden.
            Gerade bei Abhängigkeiten über einen größeren zeitlichen Abstand tendieren die Fehlersignale,
            die durch die Backpropagation durch das Netz gegeben werden, dazu zu geringe Gewichtsänderungen auszulösen.
            Traditionelle Aktivierungsfunktionen wie die hyperbolische Tangensfunktion haben Gradienten im Bereich $(-1,1)$ oder $[0,1$.
            Die Backpropagation berechnet Gradienten durch die Kettenregel.
            ...
            Die von Sepp Hochreiter erstmals erwähnte  \ac{LSTM} Zellen ermöglichen durch verbesserte Fehlerkorrektur stabilere Lernergebnisse sowie auch das Lernen von Mustern mit noch größeren zeitlichen Abständen.~\cite{HOCHREITER1998}\par\medskip

            Diese Sonderform der \acp{RNN}, die auch in dieser Arbeit verwendet werden, werden deshalb genauer untersucht.
            %% TODO Batch Normalisation \cite{LSTMbatchnorm}
   	
        \subsection{\NoCaseChange{\acfp{LSTM}}}\label{sec:LSTM}
            ...
            \begin{figure}[ht]
                ...
                \caption[Schematische Darstellung der Memory Cell eines \acp{LSTM}]{Schematische Darstellung eine Memory Cell $A$ in einem LSTM NN, mit Input, Output und Forget Gate (Abwandlung von \cite{OLAH2015}).}
                \label{fig:LSTM}
            \end{figure}
            ...
            Sie ist mit sich selbst verbunden, hat also Zugriff auf die vorherigen Ausgaben und den Zellstatus.
            Mit Hilfe dieser Information wird eine Abhängigkeit auch über einen längeren Zeitraum gefunden.
            ...
            Die Weitergabe des Status wird in~\autoref{fig:LSTM_Status} dargestellt.\par\medskip

                \begin{figure}[ht]
                    \centering
                    \includegraphics[width=0.5\textwidth]{images/Illustrationen/LSTM_MC}
                    \caption[Schematische Darstellung einer \ac{LSTM} Zelle - Zellstatus]{Weitergabe des Zellstatus innerhalb eines Knotens (Abwandlung von \cite{OLAH2015}).}
                    \label{fig:LSTM_Status}
                \end{figure}
                
                Einfluss auf den Zellstatus haben zwei verschiedene \textit{Gates}.\marginpar{zu dt. Gatter/Tore}
            Im ersten Schritt wird entschieden, welche Information aus dem vorherigen Zeitschritt keinen Einfluss mehr auf den Zellstatus haben werden.
            Dies wird mit dem \textit{Forget Gate} umgesetzt und ist in~\autoref{fig:LSTM_Forget} zu sehen und kann analog zur \ac{RNN} Zelle folgendermaßen hergeleitet werden.

            \begin{equation}
                f_t = \sigma\left(W_{fh}h_{t-1} + W_{fx}x_t + b_f\right)
            \end{equation}

            $W_{fh}$ und $W_{fx}$ beschreiben die Gewichte und $b_f$ den Bias des \textit{Forget Gate}.
            ...
            Damit werden Informationen aus dem Speicher, die keinen Einfluss mehr haben sollen, entfernt.
            ...
            Kommt nun allerdings ein neues Pronomen in der Eingabe $x_t$, sollte das bisher gespeicherte Genus keinen Einfluss mehr haben.
                ...
                \begin{figure}[ht]
                    ...
                    \caption[Schematische Darstellung einer \ac{LSTM} Zelle - Forget Gate]{Einfluss des Forget Gates auf den Zellstatus (Abwandlung von \cite{OLAH2015}).}
                    \label{fig:LSTM_Forget}
                \end{figure}
                
            Das \textit{Input Gate} gibt im nächsten Schritt an, welche neuen Informationen in den Zellstatus $C_t$ aufgenommen werden.
            Dies erfolgt in zwei Schritten, zunächst wird mit $i_t$ ermittelt, welche Information geupdated wird.
            ...
            Im Vektor $\tilde{C}$ sind mögliche Kandidaten enthalten (wie z.B. das Genus), welcher den zuvor vergessenen Wert ersetzt (vgl.~\autoref{fig:LSTM_Input}).
            ...
            \end{equation}
                ...
                \begin{figure}[ht]
                    ...
                    \caption[Schematische Darstellung einer \ac{LSTM} Zelle - Input Gate]{Einfluss des Input Gates auf den Zellstatus (Abwandlung von \cite{OLAH2015}).}
            ...
            \begin{equation}
                ...
                \end{split}
            \end{equation}
            Dies entscheidet in unserem Sprachbeispiel, ob die Information des Genus für die Vorhersage des nächsten Wortes eine Rolle spielt. \cite{GERS2000, OLAH2015}
                ...
                Die verschiedenen Gates können so als ein weiteres kleines NN in jedem Knoten der \acp{LSTM} betrachtet werden, welche einen zeitlichen Zusammenhang besser erkennen.
